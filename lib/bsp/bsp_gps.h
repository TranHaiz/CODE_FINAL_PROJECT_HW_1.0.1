...
/* Public defines ----------------------------------------------------- */
#define BSP_GPS_DEFAULT_BAUDRATE 9600

// Uncomment to enable raw NMEA debug output (only printable ASCII)
// #define BSP_GPS_DEBUG_RAW_NMEA

/* Public enumerate/structure ----------------------------------------- */

/**
 * @brief GPS data structure containing parsed position and time information
 */
typedef struct
{
...
/**
 * @brief GPS callback function type - called when valid position is received
 * @param data Pointer to GPS data structure
 */
typedef void (*bsp_gps_callback_t)(bsp_gps_data_t *data);

/**
 * @brief GPS update rate configuration
 */
typedef enum
{
 ...
 * @param config Pointer to GPS configuration structure
 ...
 * @param rate Update rate enum value
 * @return STATUS_OK on success, STATUS_ERROR on failure
 */
 ...
 * @param mode Power mode enum value
 * @return STATUS_OK on success, STATUS_ERROR on failure
 */
