...
status_function_t bsp_dust_sensor_init(void)
{
  ...
  // Warmup
  LOG_INF("Warming up dust sensor (%dms)...", BSP_DUST_SENSOR_WARMUP_MS);
  ...
  // Calibrate baseline
  LOG_INF("Calibrating baseline (%d samples)...", BSP_DUST_SENSOR_BASELINE_SAMPS);
  dust_ctx.baseline_adc = bsp_dust_sensor_calibrate_baseline();
  LOG_INF("Baseline ADC = %.2f (%.4fV)", dust_ctx.baseline_adc,
...
status_function_t bsp_dust_sensor_read(bsp_dust_sensor_data_t *data)
{
  ...
  LOG_INF("adc_avg=%.1f delta_adc=%.1f delta_v=%.4fV density=%d ug/m3", adc_avg, delta_adc, delta_v,
          data->dust_density);
