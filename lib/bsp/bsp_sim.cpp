 ...
 * @author     Hai Tran
 *
 * @brief      BSP for SIM module — ported to Quectel EG800K and A7680C
 *             Choose in device_config.h
 *
 */
...
#define MQTT_SSL_CTX      (0)
#define MQTT_SSL_SECLEVEL (1)
#define MQTT_SSL_CA_FILE  "UFS:ca.pem"
#else
#define MQTT_BROKER_PORT (1883)
#endif

#if (DEVICE_SIM_EG800K == true)
#define MQTT_CTX (0)  // EG800K MQTT context index (0-5)
#endif
...
status_function_t bsp_sim_mqtt_init(void)
{
  ...
  {
    LOG_WRN("Failed to start MQTT service: %s", sim_rx_buffer);
    return STATUS_ERROR;
  }

  bsp_sim_build_client_id(client_id, sizeof(client_id));
  snprintf(cmd, sizeof(cmd), "AT+CMQTTACCQ=0,\"%s\"\r\n", client_id);
  if (!bsp_sim_send_and_wait_response(cmd, "OK", 3000))
  {
    LOG_WRN("Failed to acquire MQTT client: %s", sim_rx_buffer);
    return STATUS_ERROR;
  }

  snprintf(cmd, sizeof(cmd), "AT+CMQTTCONNECT=0,\"tcp://%s:%d\",60,1\r\n", MQTT_BROKER_HOST, MQTT_BROKER_PORT);
  if (!bsp_sim_send_and_wait_response(cmd, "+CMQTTCONNECT: 0,0", 15000))
  {
    LOG_WRN("Failed to connect MQTT broker: %s", sim_rx_buffer);
    return STATUS_ERROR;
  }
...
status_function_t bsp_sim_mqtt_pub(mqtt_message_t *msg)
{
  ...
  if (!bsp_sim_send_and_wait_response(cmd, ">", 2000))
  {
  ...
  if (!bsp_sim_send_and_wait_response(cmd, "+CMQTTPUB: 0,0", 10000))
  {
    LOG_WRN("Failed to publish MQTT message: %s", sim_rx_buffer);
    return STATUS_ERROR;
  }
...
status_function_t bsp_sim_mqtt_sub(const char *topic, bsp_sim_mqtt_callback_t cb)
{
  ...
  snprintf(cmd, sizeof(cmd), "AT+CMQTTSUB=0,%d,%d\r\n", (int) topic_len, MQTT_SUB_QOS);
  if (!bsp_sim_send_and_wait_response(cmd, ">", 5000))
  {
    LOG_WRN("Failed to start MQTT subscribe: %s", sim_rx_buffer);
    return STATUS_ERROR;
  }
  if (!bsp_sim_send_and_wait_response(topic, "+CMQTTSUB: 0,0", 5000))
  {
    // 18 = already subscribed from a previous session — acceptable
    if (strstr((const char *) sim_rx_buffer, "+CMQTTSUB: 0,18") != NULL)
    {
      LOG_WRN("Topic already subscribed, reusing: %s", topic);
    }
    else
    {
      LOG_WRN("MQTT subscribe failed: %s", sim_rx_buffer);
      return STATUS_ERROR;
    }
  }
...
status_function_t bsp_sim_mqtt_deinit(void)
{
  ...
  {
    LOG_WRN("Failed to disconnect MQTT client: %s", sim_rx_buffer);
    ret = false;
  }
  if (!bsp_sim_send_and_wait_response("AT+CMQTTREL=0\r\n", "OK", 3000))
  {
    LOG_WRN("Failed to release MQTT client: %s", sim_rx_buffer);
    ret = false;
  }
  if (!bsp_sim_send_and_wait_response("AT+CMQTTSTOP\r\n", "OK", 5000))
  {
    LOG_WRN("Failed to stop MQTT service: %s", sim_rx_buffer);
    return STATUS_ERROR;
  }
...
status_function_t bsp_sim_mqtt_unsub(const char *topic)
{
  ...
  snprintf(cmd, sizeof(cmd), "AT+CMQTTUNSUB=0,%d,1\r\n", (int) topic_len);
  if (!bsp_sim_send_and_wait_response(cmd, ">", 5000))
  {
    LOG_WRN("MQTT unsubscribe failed: %s", sim_rx_buffer);
    return STATUS_ERROR;
  }
  if (!bsp_sim_send_and_wait_response(topic, "+CMQTTUNSUB: 0,0", 5000))
  {
    LOG_WRN("MQTT unsubscribe failed: %s", sim_rx_buffer);
    return STATUS_ERROR;
  }
...
#if (DEVICE_SIM_EG800K == true)
  // EG800K delivers subscribed messages via +QMTRECV: URC
