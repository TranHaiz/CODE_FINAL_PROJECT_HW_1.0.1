...
static sys_button_svc_state_t s_svc_state           = SVC_IDLE;
static uint8_t                s_svc_acc             = 0;
static uint32_t               s_svc_enter_ms        = 0;
static uint32_t               s_svc_last_press_ms   = 0;
...
static void sys_button_handle_event(bsp_button_press_type_t event)
{
  if (s_svc_state == SVC_COUNTING)
  {
    ...
    {
      s_svc_acc += 1;
      s_svc_last_press_ms = OS_GET_TICK();
      LOG_DBG("Service count: %d", s_svc_acc);
      break;
    }
    case BUTTON_PRESS_COUNT:
    {
      s_svc_acc += bsp_button_get_count(BUTTON_EVT);
      s_svc_last_press_ms = OS_GET_TICK();
      LOG_DBG("Service count: %d", s_svc_acc);
      break;
    }
  ...
  case BUTTON_PRESS_SERVICE:
  {
    ...
    s_svc_state         = SVC_COUNTING;
    s_svc_acc           = 0;
    s_svc_enter_ms      = OS_GET_TICK();
    s_svc_last_press_ms = s_svc_enter_ms;
...
static void sys_button_service_tick(void)
{
  ...
  if ((now - s_svc_enter_ms) >= SYS_BUTTON_SVC_MAX_MS)
  {
    LOG_DBG("Service mode timed out (max)");
    sys_button_led_oneshot(SYS_LED_EVT_SERVICE_ABORT);
    s_svc_state = SVC_IDLE;
    return;
  }

  uint32_t ref = (s_svc_acc > 0) ? s_svc_last_press_ms : s_svc_enter_ms;
  ...
  if (s_svc_acc <= SYS_BUTTON_SVC_CMD_MAX && SERVICE_CMD_MAP[s_svc_acc] != SYS_MANAGER_EVT_IDLE)
  {
    LOG_DBG("Service cmd: %d presses -> event %d", s_svc_acc, SERVICE_CMD_MAP[s_svc_acc]);
    sys_manager_write_event(SERVICE_CMD_MAP[s_svc_acc]);
    sys_button_led_oneshot(SYS_LED_EVT_SERVICE_DONE);
  }
  else
  {
    LOG_DBG("Service cmd: %d presses unmapped / out of range", s_svc_acc);
    sys_button_led_oneshot(SYS_LED_EVT_SERVICE_ABORT);
  }
