...
void sys_ui_simple_init(void)
{
  ...
  /* Set brightness */
  bsp_display_set_brightness_percent(50);
...
static void ui_main_screen_init(void)
{
  ...
  lv_label_set_text(ui_ctx.ui_temp_label, "Temp");
  ...
  lv_label_set_text(ui_ctx.ui_dust_label, "Dust");
...
static void sys_ui_simple_update_values(void)
{
  ...
  snprintf(buf, sizeof(buf), "%.2f, %.2f", lon, lat);
  lv_label_set_text(ui_ctx.ui_position_long, buf);
  snprintf(buf, sizeof(buf), "%.2f", direction);
  ...
  snprintf(buf, sizeof(buf), "%.2f", dust);
